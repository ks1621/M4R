...
def LinUCB_queue(self, la=0.5, delta=0.05/2, noise=0.5, T=1000, steps=100, q_p=0.2, q_w=1, fixed=[False, -1]):
    ...
    for t in range(T):
        ...
        for k in range(self.K):
            ...
            # Calculate queue factor
            qc = 1 + q_w * max(queue[c_index, k], 0)

            # Calculate the upper confidence bound
            p = np.dot(estimate_theta[k], context_t) * qc + alpha * np.sqrt(np.dot(context_t, A_inv @ context_t))
            p_.append(p)
