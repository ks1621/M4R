...
def LinUCB_cost(self, la=0.5, delta=0.05/2, noise=0.5, T=1000, steps=100, beta=1, fixed=[False, -1]):
    ...
    for t in range(T):
        ...
        for k in range(self.K):
            ...
            # Calculate cost
            cost = 1 + beta * (chosedcount[c_index, k]/(t+1))**2

            # Calculate the upper confidence bound
            p = np.dot(estimate_theta[k], context_t)/cost + alpha * np.sqrt(np.dot(context_t, A_inv @ context_t))
            p_.append(p)
