...
def LinUCB_fairbounds(self, la=0.5, delta=0.05/2, noise=0.5, T=1000, steps=100, wid=0.5, fixed=[False, -1]):
    ...
    for t in range(T):
        ...
        for k in range(self.K):
            ...
            u = np.dot(estimate_theta[k], context_t) + alpha * np.sqrt(np.dot(context_t, A_inv @ context_t)) 
            l = np.dot(estimate_theta[k], context_t) - alpha * np.sqrt(np.dot(context_t, A_inv @ context_t)) * wid
            u_.append(u)
            l_.append(l)
